 ...
 if(function_exists('current_user_can') && current_user_can('administrator')){
	...
	if(isset($_POST['saveorder'])){
		...
		$SQL = "UPDATE ".$wpdb->prefix."orderdata SET 
			...
			order_country='".mysql_real_escape_string($_POST['order_country'])."',
			...
			if(isset($_POST['order_status']) && $_POST['order_status'] == 3){			
			// SEND USER EMAIL
			$emailID = get_option("email_order_after");			 
			if(is_numeric($emailID) && $emailID != 0){				 
				SendMemberEmail($_POST['cus_id'], $emailID);	 						
			}
	...
	// PUT BACK ORDER QTY
	$SQL = "SELECT order_items FROM ".$wpdb->prefix."orderdata WHERE ".$wpdb->prefix."orderdata.order_id = '".PPTCLEAN($_POST['delo'])."' LIMIT 1"; 
	$posts = mysql_query($SQL, $wpdb->dbh) or die(mysql_error().' on line: '.__LINE__);
	while ($order = mysql_fetch_object($posts)) {
	
	$bb = explode(",",$order->order_items);
	foreach($bb as $product){
	 
		$aa = explode("x",$product);
		$current =  get_post_meta($aa[0], 'qty', true);
		if(is_numeric($aa[1])){
			$current = $current + $aa[1];
			update_post_meta($aa[0], 'qty', $current);
		}
	}
...
if(isset($_GET['id'])  ){ 
	...
	<h3><img src="<?php echo PPT_FW_IMG_URI; ?>/admin/new/block7.png" align="middle"> #<?php echo $order->order_id ?></h3>							 
	<ul>
	...
	if(!empty($product_array)){ ?>
	 ...
	 <table width="100%"  class="maintable" style="style="style="background:white;""">
...
<td colspan="3"><p><input class="premiumpress_button" type="submit" value="Save Changes" style="color:#fff;" /> <a href="<?php echo $GLOBALS['template_url']; ?>/admin/_invoice.php?id=<?php echo $order->order_id ?>" class="premiumpress_button" style="background:#1687f0;" target="_blank">View Invoice</a></p>
	...
	<table class="maintable" style="style="style="background:white;""">
...
<td colspan="3"><p><input class="premiumpress_button" type="submit" value="Save Changes" style="color:#fff;" /></p></td>
	...
	<table class="maintable" style="background:white;" > 
 		...
 		<tr class="mainrow">
          ...
          <p>This field will be populated with any POST data sent back by the payment gateway or initial listing setup, this is used for testing and reference only.</p>
...
$params['sql_query']				= 'SELECT order_id,order_date,cus_name,order_total,order_status FROM '.$wpdb->prefix.'orderdata WHERE order_status='.$_GET['status'];
	...
	if(get_option("orders_awaitingpayment") == "1"){
	$params['sql_query']				= 'SELECT order_id,order_date,cus_name,order_total,order_status FROM '.$wpdb->prefix.'orderdata WHERE order_id !="" ORDER BY order_date desc';
	}else{
	$params['sql_query']				= 'SELECT order_id,order_date,cus_name,order_total,order_status FROM '.$wpdb->prefix.'orderdata WHERE order_id !=""  AND order_status != 0  ORDER BY order_date desc';
	}
...
$params['search_html']				= '<span id="#ID#_search_value">Search Orders...</span> <div id="#ID#_loader"></div>';
...
if(isset($_GET['ajaxcall'])){
	...
	$params['sql_query'] = str_replace('WHERE order_id !="" ORDER BY order_date desc',"",$params['sql_query']);
	$ct->table($params);
...
<h3><img src="<?php echo PPT_FW_IMG_URI; ?>/admin/_ad_orders.png" align="middle"> Order Management</h3> <a class="premiumpress_button" href="javascript:void(0);" onclick="PPHelpMe()"><img src="<?php echo PPT_FW_IMG_URI; ?>/admin/youtube.png" align="middle" style="margin-top:-10px;"> Help Me</a>	
<ul>
...
<fieldset style="padding:0px;border:0px;"><?php echo $out;?></fieldset>
<a href="admin.php?page=orders&exportcsv=1" class="button" style="float:right;">Export Sales To CSV</a>
...
<form method="post" target="_self" name="ap" id="ap" enctype="multipart/form-data">
<input name="submitted" type="hidden" value="yes" />
<p><input name="df1" type="checkbox" value="1" onchange="ChangeTickValue('df1'); document.ap.submit();" <?php if(get_option("orders_awaitingpayment") == "1"){ ?>checked="checked"<?php } ?> /> Include Awaiting Payment</p>
<input type="hidden" name="adminArray[orders_awaitingpayment]" id="df1" value="<?php if(get_option("orders_awaitingpayment") ==1){ echo 1; }else{ echo 0; } ?>" />
</form>
